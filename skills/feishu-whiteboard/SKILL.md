...
> [!NOTE]
> **环境依赖**：绘制画板需要 `@larksuite/whiteboard-cli`（画板 Node.js CLI 工具）。
...
> 上传需要飞书认证。遇到认证或权限错误时，阅读 [`../feishu-troubleshoot/SKILL.md`](../feishu-troubleshoot/SKILL.md) 了解登录和权限处理。
...
|------------|--------------|
| 画板 Token（`XXX`） | 直接使用 |
| 文档 URL 或 doc_id，文档中已有画板 | 使用 `feishu_mcp_fetch_doc` 工具获取文档内容，从返回的 `<whiteboard token=”XXX”/>` 中提取 token |
| 文档 URL 或 doc_id，需要新建画板 | 使用 `feishu_update_doc` 工具（mode: append，markdown: `<whiteboard type=”blank”></whiteboard>`），从响应的 `data.board_tokens[0]` 获取 token |

关于飞书文档的创建，读取等更多操作，请参考 feishu-fetch-doc skill [`../feishu-fetch-doc/SKILL.md`](../feishu-fetch-doc/SKILL.md) 和 feishu-create-doc skill [`../feishu-create-doc/SKILL.md`](../feishu-create-doc/SKILL.md)。
